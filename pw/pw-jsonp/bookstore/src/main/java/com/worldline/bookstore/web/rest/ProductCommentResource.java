...
public class ProductCommentResource {
    ...
    public ResponseEntity<ProductComment> createProductComment(@Valid @RequestBody ProductComment productComment) throws URISyntaxException {
        ...
        if (productComment.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new productComment cannot already have an ID")).body(null);
        }
     ...
     * or with status 500 (Internal Server Error) if the productComment couldnt be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
...
/*    @GetMapping("/product-comments/{id}")
    ...
    public ResponseEntity<ProductComment> getProductComment(@PathVariable Long id) {
        ...
        ProductComment productComment = productCommentRepository.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(productComment));
    }*/
